...
class PurchaseOrderRepository{
  ...
  public static function set_purchase_order($connection, $responsible, $date, $order_date, $purchase_from, $drop_ship_to, $comments, $po_number, $ref_quote, $ship_via, $terms, $message, $id_purchase_order){
    if(isset($connection)){
      try{
        $sql = 'UPDATE purchase_orders SET responsible = :responsible, date = :date, order_date = :order_date, purchase_from = :purchase_from, drop_ship_to = :drop_ship_to, comments = :comments, po_number = :po_number, ref_quote = :ref_quote, ship_via = :ship_via, terms = :terms, message = :message WHERE id = :id_purchase_order';
        $sentence = $connection-> prepare($sql);
