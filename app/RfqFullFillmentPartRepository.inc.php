...
class RfqFullFillmentPartRepository{
  public static function insert_rfq_fullfillment_part($connection, $rfq_fullfillment_part){
    if(isset($connection)){
      try{
        ...
        $sentence-> bindParam(':payment_terms', $rfq_fullfillment_part-> get_payment_terms(), PDO::PARAM_STR);
        $sentence-> execute();
  ...
  public static function save_rfq_fullfillmet_info($connection, $name, $po_number, $business_classification, $description, $po_date, $eta, $consolidate_others, $total_vendor_cost, $fedbid, $estimated_final_cost, $estimated_profit_g, $percent_g, $estimated_profit_s, $percent_s, $payment_terms, $id_rfq_fullfillment_part){
    if(isset($connection)){
      try{
        $sql = 'UPDATE rfq_fullfillment_part SET name = :name, po_number = :po_number, business_classification = :business_classification, description = :description, po_date = :po_date, eta = :eta, consolidate_others = :consolidate_others, total_vendor_cost = :total_vendor_cost, fedbid = :fedbid, estimated_final_cost = :estimated_final_cost, estimated_profit_g = :estimated_profit_g, percent_g = :percent_g, estimated_profit_s = :estimated_profit_s, percent_s = :percent_s, payment_terms = :payment_terms WHERE id = :id_rfq_fullfillment_part';
        $sentence = $connection-> prepare($sql);
