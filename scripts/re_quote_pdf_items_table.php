  ...
  if (count($re_quote_items)) {
    ...
    for ($i = 0; $i < count($re_quote_items); $i++) {
      ...
      for($j = 0; $j < count($re_quote_subitems); $j++){
        ...
        ');
        $mpdf->WriteHTML($html);
        $mpdf->Output($_SERVER['DOCUMENT_ROOT'] . '/fullfillment/documents/rfq_team/' . $cotizacion->obtener_id() . '/' . preg_replace('/[^a-z0-9-_\-\.]/i','_', $cotizacion-> obtener_email_code()) . '(re_quote_items_table)' . '.pdf', 'F');
